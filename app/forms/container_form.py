...
class FoodItemForm(FlaskForm):
    container_id = IntegerField("Container ID", validators=[DataRequired()])
    food_id = IntegerField("Food ID", validators=[DataRequired()])
    amount = StringField("Amount", validators=[Optional()])
    expiration = DateField("Expiration Date", validators=[Optional()])  # Optional for non-perishable item
    ...
    #TODO: (shouldn't happen) Validate container exists
